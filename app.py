...
with st.sidebar:
    ...
    st.markdown("<div style='color:#8b9ab5;font-size:0.8rem;font-weight:600;margin-bottom:0.4rem;margin-top:1rem'>🌡️ CREATIVITY</div>", unsafe_allow_html=True)
    ...
    if st.button("🎯 Run BA Analysis", type="primary", key="req_run"):
        if scenario.strip():
            ...
            with st.spinner(f"🤖 {num_agents} BA agent(s) analyzing..."):
                clean_role = babok_area.split(" ", 1)[1]
                result_str = run_crew(final_scenario, clean_role, num_agents, model_choice, temperature)
            ...
            st.warning("⚠️ Please describe your scenario first!")

with tab2:
    st.markdown("#### 📎 Document Analyzer")
    st.markdown("<div style='color:#8b9ab5;font-size:0.9rem;margin-bottom:1rem'>Upload BRDs, SOWs, contracts, meeting notes, process docs — any business document.</div>", unsafe_allow_html=True)
    uploaded_files = st.file_uploader("Upload Documents", type=["pdf","docx","pptx","png","jpg","jpeg","txt"], accept_multiple_files=True, label_visibility="collapsed")
    analysis_type = st.selectbox("What do you want to extract?", [
        "📋 Extract & Summarize Requirements",
        "👥 Identify Stakeholders",
        "⚠️ Flag Risks & Assumptions",
...
with tab3:
    ...
    ba_templates = {
        ...
        "⚠️ Risk Register": "Create a BA risk register for an ERP implementation project. Include risk description, probability, impact, mitigation strategy, and owner for each risk.",
    ...
    if st.button("🛠️ Generate BA Deliverable", type="primary", key="ba_run"):
        if ba_scenario.strip():
            with st.spinner("🤖 CBAP-certified BA agent generating deliverable..."):
                result_str = run_crew(ba_scenario, "Senior Business Analyst", 1, model_choice, temperature)
                st.session_state.run_count += 1
                st.session_state.history.append({
                    "type": "BA Toolkit",
                    "tab": "BA Toolkit",
...
with tab4:
    ...
    if not st.session_state.history:
        ...
        for i, item in enumerate(reversed(st.session_state.history)):
            ...
            icons = {"Requirements": "🎯", "Document": "📎", "BA Toolkit": "🛠️"}
            icon = icons.get(item["tab"], "🎯")
            with st.expander(f"{icon} Run #{run_num} — {item['type']} · {item['time']}"):
                tag_html = f"<span class='sim-tag'>📁 {item['tab']}</span><span class='sim-tag'>🤖 {item['type']}</span><span class='sim-tag'>🕒 {item['time']}</span><span class='sim-tag'>⚙️ {item.get('model','N/A')}</span>"
